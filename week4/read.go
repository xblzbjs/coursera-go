...
// Name return firstname and lastname
type Name struct {
	fname string
	lname string
}
...
func main() {
	...
	for {
		...
		n.fname = strSlice[0]
		n.lname = strSlice[1]
		if len(n.fname) > maxLength {
			n.fname = n.fname[:maxLength]
		}

		if len(n.lname) > maxLength {
			n.lname = n.lname[:maxLength]
		}
		nameList = append(nameList, n)
	}
	for _, elt := range nameList {
		fmt.Printf("%s %s\n", elt.fname, elt.lname)
	}
